...
The first milestone does not require structural collapse, spreading fire or full dynamic multi-level fluids. Generated rivers and deep pools already occupy real lower-level volume. Breaching a water bed or the final wall now derives a same-level flooded region from the immutable source volume and saved excavation topology, without serializing a presentation cache. This deliberately does not yet simulate finite volume, pressure, vertical falls, flow rates, evaporation or constructed hydraulic barriers. Intact solids may later transmit bounded moisture or heat clues from a hidden neighboring fluid body without exposing that body's cells. Drainage, diversion, sumps and later pumps must conserve fluid and provide a physical destination instead of deleting water through an abstract dry-room command.
...
Area tools have explicit selection semantics rather than one accidental rectangle behavior. Filtered gathering tools pass the complete dragged area to an authoritative simulation query and retain only concrete matching targets such as live reed beds, harvestable plants, loose brushwood, mineral stacks, trees, boulders or animals. The same query drives both the preview and command execution, so empty cells disappear immediately and UI filtering cannot diverge from the dispatcher. Apply-to-area tools such as excavation, scouting and blood cleaning retain every currently applicable cell; clearing work designations applies to the complete selected region. Single-cell tools such as ramp carving use the same pipeline with a one-cell policy. Future fields, stockpiles and dumping zones are multi-cell objects whose selected footprint belongs to one stable object identity rather than a bag of unrelated clicks. New objects entering an old filtered-gathering rectangle are not enrolled automatically, and repeating the same work kind over the same concrete target does not duplicate it.
...
The first village dispatcher uses population and expected yield to plan field area. Workers draw water from the well, clear nearby land with wooden axes and accumulate timber; farmers sow, tend and harvest with wooden hoes. Food is stored as grain-equivalent and daily consumption abstracts milling and baking flatbread. The demo-temperate economy profile grows a field for 20 simulation days, giving two harvest opportunities in its 40-day year. Need rates, meal and drink thresholds, rest rates, deprivation damage, crop duration, yield and storage capacities are grouped definition data rather than constants hidden in the village state. Temperature, rainfall, frost, soil exhaustion and additional climate economies remain later ecology layers. Any capacity added locally must come from a constructed spatial object; the initial human storehouse is a physical 3×3 building rather than an aggregate bonus.
...
The post-foundation underground expansion uses the same neighbor boundary vertically. No intelligent deep faction is simulated above `z=-6`. Every subsequent ten-level band may deterministically contain a fortress with an aggregate tribal dispatcher and inter-faction relations. Descending into a band activates that record; detailed inhabitants and structures are materialized only when local interaction requires them. Hostile active factions can resolve coarse warfare before the player encounters either side.

The current neighbor slice has begun that transition. All twelve residents in the active chunk are now materialized as deterministic individuals with stable identity, position, role, skill, concrete wooden tool, health, fatigue, hunger and thirst. They move separately while receiving role-level work from the village dispatcher; save-format 52 migrates former cohort-only saves and preserves individual, field, felling, workshop and storehouse-construction progress. Food and water are allocated to the neediest concrete residents when their personal thresholds are reached, deprivation damages and can kill that exact person, and night or severe fatigue sends villagers back to rest near the settlement. Water is the first stock production converted from a cohort grant: only a worker carrying a bucket can accumulate saved work while standing at one of the physical access cells around the well, and completed work becomes village stock at dawn. Farming is likewise physical: a farmer with a wooden hoe must reach a concrete field and finish its saved stage progress before the next dawn can sow it, advance one growth day or harvest it. An unattended field remains in its current stage rather than growing from the calendar alone. When timber reserves fall below the configured construction reserve, the dispatcher selects one reachable nearby tree or stump. Workers carrying wooden axes approach its physical trunk, contribute to one saved shared felling job and use the ordinary world mutation on completion: a tree becomes a stump, a stump disappears and only the deterministic real yield enters the village stock. Planning another field no longer grants wood merely because worker records exist. Basic goods are no longer an automatic seven-day conversion either. If their population-scaled reserve is low and enough timber remains above the first-storehouse reserve, an axe-equipped worker enters a traversable floor cell inside the physical barn and contributes to saved workshop progress. A completed unit consumes the configured timber cost immediately and adds one aggregate good. A storehouse likewise begins as a deterministic saved 3x3 construction site, not a dawn-time capacity grant. Workers travel into its footprint and contribute shared construction work; only completion consumes the timber, creates the exact physical building and increases food capacity. Material delivery to that site remains the next logistics slice. Human combat likewise targets concrete living guards: each attacks from its own position, receives its own damage and produces blood plus a persistent corpse carrying its stable victim identity and physical tool contents. Goblin death creates the same kind of searchable container and retains equipped items, provisions, ammunition and carried cargo inside the body instead of transferring them to the ground. Save-format 55 introduces that corpse state. The first equipment projection now names body, weapon, ammunition, belt, waterskin and backpack slots and reports equipment, pack and cargo weight; replacing the legacy flag representation with concrete item identities remains the next inventory slice. The aggregate guard total remains only a compatibility and overview projection. The three cohort records likewise remain temporarily as dispatcher summaries. Before the neighbor slice can be called complete, aggregate human stocks must become physical inventories and cohort records must move exclusively to unloaded chunks and traveling groups outside the active area.
...
### Milestone 2 raid delivery slices

1. **Bodies and loadouts:** persistent searchable corpse containers, concrete equipment identities, slots, weight and carrying capacity.
2. **Village stores:** physical food, materials, tools and finished goods placed in plausible buildings and containers instead of raid-visible aggregate counters.
3. **Engagement:** target-radius enforcement, guard-only versus non-fleeing targets, pursuit limits, retreat and return to the rally camp. The current doctrine foundation distinguishes concrete guards from civilians: ordinary encounters and guard-only raids strike defenders, while an actively marching party configured to attack non-fleeing people may also strike a concrete civilian inside the selected raid radius. Human and goblin attack phases resolve independently, so sleeping, fleeing or eliminated guards do not suppress an otherwise valid goblin attack. Pursuit limits and voluntary retreat remain.
4. **Looting:** explicit search work, item removal from a corpse or village container, physical carried cargo and delivery to camp or tribal storage. Corpse and in-radius building extraction now deliver bounded stacks to the rally camp and persist their economic/tool losses; player-directed destinations remain.
5. **Corpse recovery:** human bodies retain eight saved edible portions and a biological imprint of the carrier's available knowledge; the consumption directive performs one physical raw-meat meal at the body per work cycle. A separate corpse order either recovers the body unchanged, recovers and pollinates it at the rally camp, or pollinates it where it fell. The goblin completing pollination becomes one inheritance source and the corpse imprint becomes the other. Selected alternative substrates remain.
6. **Damage to place:** optional demolition and fire applied to concrete structures, followed by persistent village population, stock and hostility consequences.
7. **Knowledge target:** observation, capture or consumption of one notable carrier and the first partial stolen capability.

The current implementation covers the first physical aftermath path across slices 1–5: corpse contents, meat, biological imprints and village stores are inspectable simulation state; selected stacks are removed and carried; raiders consume saved portions in place; and explicit recovery, recovery-plus-pollination or in-place pollination orders preserve their distinct outcomes. A corpse-origin bud records its pollinating goblin and the victim's separate imprint through save/load, then resolves partial inheritance from both when it matures. Surviving raiders walk back before victory closes. Guard-only and non-fleeing engagement select concrete people according to the active doctrine and raid radius. Concrete item identities, player-selected storage destinations, pursuit and retreat policy and the remaining doctrine outcomes still need later slices.
