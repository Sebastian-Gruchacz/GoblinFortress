...
The sandbox starts with one food stockpile at the goblin spawn and deterministic loose brushwood scattered across fertile land, including a small reachable starter supply. Left-click a visible goblin to select it and open its resizable, scrollable detail window, then right-click any map cell to order a move, including a scouting move into unknown territory. Left-click a stockpile to configure whether it requests loose resources and the target quantity. **Praca…** provides draggable gather-food, gather-brushwood, uproot-bush and clear-designation tools; food targets include berries, mushrooms, edible roots and fish shoals. Harvested berry bushes remain visible without fruit and regrow berries because the current sandbox is fixed in summer. Uprooting permanently removes the bush and earns building experience. Mining and tree-felling are visible but disabled until those world resources exist. Satiated goblins do not gather wild food without a designation, while hunger may still create emergency foraging. With no goblin selected, right-click opens the same construction menu as **Buduj…**. Food and wood stockpiles each cost 2 wood. A wooden walkway costs 1 wood per cell and is laid by dragging from its first to its last cell; it may cross both land and water. Escape cancels the active tool.
...
- explicit floor elevation per terrain column: shallow water has a surface-level floor, while generator v3 deep water exposes a submerged floor at `z=-1` and is impassable to default humans and goblins;
...
- explicit construction commands that atomically consume physical wood or reject without partial changes;
...
- per-stockpile loose-resource pulling with an explicit on/off state and target quantity;
...
- work designations and stockpile demand included in snapshots, deterministic hashes and saves.
...
Generated structures currently validate the spatial contract but are static: they have no materials, construction progress, damage, fire or functional rooms yet. Player construction is still an atomic prototype: visible loose brushwood or stored wood is consumed and the finished stockpile or walkway appears on the execution tick. The nearest goblin is recorded as the abstract builder and gains experience; delivery to a construction site, visible work progress and interruption recovery remain the next construction slice. Using loose brushwood grants gathering experience as part of this abstract task. Walkways cannot share a surface-occupancy cell with another structure, and their current drag line is a deterministic cardinal staircase. Actual trees, felling, renewable deadwood, streams and river networks are not generated yet; fertile traversable ground is only a deterministic proxy for initial brushwood placement. Skill levels are persisted and visible but do not modify speed or yield yet.

The dispatcher stores one active job plus one suspended resumable intent per goblin. Eating, drinking or rest can temporarily displace ordered movement, exploration, gathering or clearance; after the need is satisfied the target is revalidated and the route is rebuilt. Dynamic hauling releases its reservations and is selected again from the public work pool instead of preserving stale ownership. Multi-action personal plans, rising need scores, plan hysteresis and soft reservations for later actions remain the next scheduling layer. One autonomous explorer still operates without a player work area. Stockpile pulling currently handles loose ground stacks and bounded target quantities; priorities, protected reserves, source restrictions and inter-stockpile routes remain in the deliberate-logistics milestone.
