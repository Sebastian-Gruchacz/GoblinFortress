...
Manual transport commands still resolve atomically after a path check. Autonomous foraging and hauling use explicit routes and action duration. The current hauler selects the shortest valid source/destination pair and respects filters, carry capacity and physical reservations. Construction demand competes ahead of ordinary stockpile pulling and may consume reachable loose or stored wood. The dispatcher does not yet implement player-defined local/global priorities, reserve floors or general workshop demand.
